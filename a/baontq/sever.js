...
async function getGameKey(env, bundleId) {
  console.log(`[getGameKey] ===== START =====`);
  console.log(`[getGameKey] bundleId: "${bundleId}"`);
  console.log(`[getGameKey] bundleId type: ${typeof bundleId}`);
  console.log(`[getGameKey] bundleId length: ${bundleId?.length}`);
  
  if (!bundleId) {
    console.log(`[getGameKey] bundleId is empty, returning default`);
    return DEFAULT_AES_KEY_HEX;
  }
  
  // Kiểm tra KV binding
  console.log(`[getGameKey] KV binding exists: ${!!env.KEY_STORE_2}`);
  
  const games = await env.KEY_STORE_2.get("registered_games");
  console.log(`[getGameKey] Raw from KV: "${games}"`);
  
  if (!games) {
    console.log(`[getGameKey] No data in KV, returning default`);
    return DEFAULT_AES_KEY_HEX;
  }
  
  const gameList = JSON.parse(games);
  console.log(`[getGameKey] Parsed gameList:`, JSON.stringify(gameList));
  
  for (const [gameId, aesKey] of Object.entries(gameList)) {
    const matches = bundleId.includes(gameId);
    console.log(`[getGameKey] Check: "${bundleId}" includes "${gameId}"? ${matches}`);
    if (matches) {
      console.log(`[getGameKey] ✅ MATCH! Returning key: ${aesKey}`);
      return aesKey;
    }
  }
  
  console.log(`[getGameKey] ❌ NO MATCH, returning default: ${DEFAULT_AES_KEY_HEX}`);
  return DEFAULT_AES_KEY_HEX;
}
...
export default {
  async fetch(request, env) {
    ...
    // ========== API Lấy key theo bundle_id ==========
    if (path === "/api/v1/get_key") {
      const { bundle_id } = body;
      
      if (!bundle_id) {
        return await json({ error: "Missing bundle_id" }, 400);
      }
      
      const aesKey = await getGameKey(env, bundle_id);
    ...
    // ========== API Thêm game mới ==========
    if (path === "/api/v1/add_game") {
      const { game_id, aes_key } = body;
      
      if (!game_id || !aes_key) {
        return await json({ error: "Missing game_id or aes_key" }, 400);
      }
      
      // Lấy danh sách game hiện tại
      let games = await env.KEY_STORE_2.get("registered_games");
      let gameList = games ? JSON.parse(games) : {};
      
      // Thêm game mới
      gameList[game_id] = aes_key;
      
      // Lưu lại
      await env.KEY_STORE_2.put("registered_games", JSON.stringify(gameList));
      
      return await json({
        success: true,
        message: `Game ${game_id} added successfully`,
        game_id: game_id,
        aes_key: aes_key
      });
    }
    ...
    // ========== API Xóa game ==========
    if (path === "/api/v1/remove_game") {
      const { game_id } = body;
      
      if (!game_id) {
        return await json({ error: "Missing game_id" }, 400);
      }
      ...
      return await json({
        success: true,
        message: `Game ${game_id} removed successfully`
      });
    }
    
    // ========== API Liệt kê tất cả game ==========
    if (path === "/api/v1/list_games") {
      const games = await env.KEY_STORE_2.get("registered_games");
      const gameList = games ? JSON.parse(games) : {};
      
      const result = [];
      for (const [gameId, aesKey] of Object.entries(gameList)) {
        result.push({ game_id: gameId, aes_key: aesKey });
      }
    ...
    if (path === "/public/v1/client/package") {
      ...
      const KEY_HEX = await getGameKey(env, bundle_id);
      console.log(`[Server] Package - bundle_id: ${bundle_id}, using key: ${KEY_HEX}`);
    ...
    if (path === "/public/v1/client/check") {
      const { token, data, bundle_id } = body;

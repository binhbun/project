...
// ==================== AES Crypto Helpers ====================
static void getAesKeyFromServerAsync(NSString *bundleId, void(^completion)(NSString *key)) {
    ...
    NSURLSessionDataTask *task = [[NSURLSession sharedSession] dataTaskWithRequest:request 
        ...
        if (error || !data) {
            NSLog(@"[Dylib] Error fetching key: %@", error);
            if (completion) completion(@"bf76c74c23bd93c4016a2a0be4213f63");
            return;
        }
        ...
        if (!jsonError && [json[@"success"] boolValue]) {
            NSString *key = json[@"aes_key"];
            verifiedBundleId = bundleId;
            if (key && completion) completion(key);
        } else {
            if (completion) completion(@"bf76c74c23bd93c4016a2a0be4213f63");
        }
    }];
...
__attribute__((constructor)) static void initialize_complete_bypass() {
    @autoreleasepool {
        ...
        getAesKeyFromServerAsync(currentBundleId, ^(NSString *key) {
            if (key) {
                if (!keyCache) keyCache = [NSMutableDictionary dictionary];
                keyCache[currentBundleId] = key;
                NSLog(@"[Dylib] AES Key loaded: %@", key);

...
static void LogFullJSON(NSString *jsonString) {
    // Chia nhỏ mỗi đoạn 400 ký tự (để đảm bảo không bị hệ thống cắt)
    int chunkSize = 400;
    int length = (int)jsonString.length;
    
    NSLog(@"[AppLogger] --- Bắt đầu JSON ---");
    for (int i = 0; i < length; i += chunkSize) {
        int range = MIN(chunkSize, length - i);
        NSString *chunk = [jsonString substringWithRange:NSMakeRange(i, range)];
        // Dùng log riêng để trên 3uTools dễ lọc
        NSLog(@"[AppLogger] CHUNK: %@", chunk);
    }
    NSLog(@"[AppLogger] --- Kết thúc JSON ---");
}
...
static void logHexInChunks(NSString *prefix, const void *bytes, size_t length) {
    // In ra nhiều dòng nhỏ, mỗi dòng tối đa 48 byte (144 ký tự)
    const uint8_t *b = (const uint8_t *)bytes;
    size_t chunkSize = 48; // 48 byte * 3 ký tự = 144 ký tự, an toàn
    size_t offset = 0;
    while (offset < length) {
        ...
        for (size_t i = 0; i < thisChunk; i++) {
            [hexStr appendFormat:@"%02x ", b[offset + i]];
        }
        NSString *trimmed = [hexStr stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
        if (offset == 0) {
            NSLog(@"[AppLogger] %@[%zu]=%@", prefix, length, trimmed);
        } else {
            NSLog(@"[AppLogger] %@(cont.) %@", prefix, trimmed);
        }
        offset += thisChunk;
    }
}
    ...
    void *out, size_t outAvail, size_t *outMoved)
{
    ...
    ALOG(@"🔐 CCCrypt | %@ | %@ | PKCS7=%@ ECB=%@ | key[%zu]=%@ | in[%zu]=... | out[%zu]=... | status=%d",
         ...
         keyLen, hexBytes(key, keyLen),   // key thường ngắn nên in trực tiếp
         inLen, 
         outMoved ? *outMoved : 0,
         (int)st);
    
    // In input và output thành từng chunk nhỏ
    logHexInChunks(@"in", in, inLen);
    if (st == kCCSuccess && outMoved) {
        logHexInChunks(@"out", out, *outMoved);
    }
...
+ (id)al_JSONObjectWithData:(NSData *)data options:(NSJSONReadingOptions)opt error:(NSError **)err {
    ...
    if (jsonStr) {
        LogFullJSON(jsonStr); // Gọi hàm chia nhỏ log ở đây
    }
